                        ...
                        @if ($question->options->count() > 0)
                            @foreach ($question->options as $option)
                                ...
                                <div class="progress" style="height: 30px;">
                                    <div class="progress-bar @if ($totalAnswers > 0 && ($optionAnswers / $totalAnswers) * 100 == 0) ml-2 text-dark @endif" role="progressbar" style="width:{{ $totalAnswers > 0 ? ($optionAnswers / $totalAnswers) * 100 : 0 }}%;"
                                        aria-valuenow="{{ $optionAnswers }}" aria-valuemin="0" aria-valuemax="{{ $totalAnswers }}">{{ $optionAnswers }}</div>
                                </div>
                            @endforeach
                        @else
